...
app.get("/user/list", async function (request, response) {
  try {
    const users = await User.find({}, '_id first_name last_name'); // Specify minimum fields to include
    return response.status(200).json(users);
  } catch (error) {
    console.error("Error retrieving users:", error);
    return response.status(400).send("Error retrieving users");
  }
});
...
app.get("/user/:id", async function (request, response) {
  ...
  try {
    const user = await User.findById(id)
      .select("-__v"); // Deselect version field
...
app.get("/photosOfUser/:id", async function (request, response) {
  ...
  try {
    const photos = await Photo.find({ user_id: userId })
      .select("-__v") // Deselect the version field
      .populate({ // Create the user object with minimum fields
    ...
    if (photos.length === 0) {
      ...
      return response.status(400).send("Photos for user with _id:" + userId + " not found.");
    }

    const photosWithRenamedUser = photos.map(photo => ({
      ...photo.toObject(),
      comments: photo.comments.map(comment => ({
        ...comment.toObject(),
        user: comment.user_id,
        user_id: undefined,  // remove old user_id field
      })),
    }));

    return response.status(200).json(photosWithRenamedUser);
  } catch (error) {
    console.error("Error retrieving photos:", error);
    return response.status(400).send("Error retrieving photos for user with _id: " + userId);
  }
});
